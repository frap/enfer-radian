  ...
  (cond
   ...
   (t
    ...
    (defvar radian-local-init-file
      ...
      "File for local customisations of Radian.")
    ...
    (if (version< emacs-version radian-minimum-emacs-version)
        (error (concat "Radian Emacs nécessite au moins Emacs %s, "
                       "mais vous utilisez Emacs %s")
      ...
      (let ((link-target
        ...
        (unless link-target
          (error "Init-file %S est pas un symlink" this-file))

        (defvar radian-lib-file (expand-file-name
                                 "radian.el"
                                 (file-name-directory link-target))
          "File containing main Enfre Radian configuration.
This file is loaded by init.el.")

        (unless (file-exists-p radian-lib-file)
          (error "Fichier de bibliothèque %S n'existe pas"))
