...
;;; Define Radian customisation groups
...
(defgroup radian nil
  "Customise your Radian Emacs experience."
  :prefix "radian-"
  :group 'emacs
  :link '(url-link :tag "GitHub" "https://github.com/raxod502/radian"))
...
`defun'."
  (declare (indent 2)
           (doc-string 5))
  (unless (stringp docstring)
    (error "Radian: pas de docstring prévu pour `radian-defadvice'"))
  `(progn
     (eval-and-compile
       (defun ,name ,arglist
...
as in `defun'."
  ...
  (unless (stringp docstring)
    (error "Radian: pas de docstring prévu pour `radian-defhook'"))
...
;;  (let ((user-emacs-directory enfer-local-dir))
;;    (apply orig-fn args)))
...
(use-package no-littering
  :demand t
  :init
  (setq no-littering-etc-directory enfer-etc-dir)
  (setq no-littering-var-director enfer-cache-dir))
                ...
                (let* ((results (split-string
                  ...
                  (if (cl-evenp (length results))
                    ...
                    (message
                     ...
                     profile-file
                     buf-name)))
              (message "Échec du chargement %s; see buffer %S"
                       profile-file
                       buf-name))))))))
  ...
  (when (and
    ...
    (cl-dolist (dir-to-delete radian--dirs-to-delete)
      ...
      (when (and (stringp dir-to-delete)
        ...
        (if (y-or-n-p (format "Supprimer également le répertoire `%s'? "
...
;; Package `dns-mode' provides a major mode for dns files.
...
(defun radian-reload-init ()
  ...
  (interactive)
  (message "Rechargement init-file...")
  ...
  (message "Rechargement init-file...fini"))
